...
class Logger:
    def __init__(self) -> None:
        ...
        self.__file = open(f"poe_client.log", "a")
    ...
    def __call__(self, message):
        try:
            ...
            if type(message) == list:
                message = "\n".join(message)
            self.__file.write(f"[{strftime('%Y-%m-%d %H:%M:%S')}] <{python_filename}|{python_function}|{python_line}>\n{message}\n")
            self.__file.flush()
        except:
            if hasattr(message, "__repr__"):
                ...
                self.__call__(f"<!>(no __repr__ or __str__ method found)")